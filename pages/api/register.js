...
handler.post(async (req, res) => {
	const {username, email, password, confirm} = req.body; // get email and password from request body
	if(!username) throw new Errors.BadRequestError("Username is required."); // throw error 400 when username is not present 
	...
	if(password !== confirm) throw new Errors.BadRequestError("Passwords do not match"); // throw error 400 when confirmed password is not eqault to password
	
	
    const salt = await bcrypt.genSalt(10) // generates salt for crypting a password
    const hashed = await bcrypt.hash(password, salt) // crypting password with given salt

	const insertResult = await User.Create({username, email, password: hashed}); // inserts user into the database
	if(insertResult.error) throw insertResult.error; // if there is an error while inserting throw an error
