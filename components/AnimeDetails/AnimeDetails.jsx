...
export default function AnimeDetails({anime, setAnime}) {
	return (
		<>
			...
			<h1>{anime.name}</h1>
			<p>{anime.rating ?? "0.0"} <TiStarFullOutline /></p>
			<p>{anime.released}</p>
			...
			<h2>Select episode to watch:</h2>
			{
				anime.episodes.length > 0 ? (
					anime.episodes.map(episode => <p key={episode.id} ><a href={`/${anime.id}/episode/${episode.orderNumber}`}>Episode  {episode.orderNumber} {(episode.watched ? "+" : "")}</a></p>)
				) : (
